...
(defun run-expert-selection-inspection-tests ()
  (let* ((precondition
           (hackmode:make-expert-objective-clause
            :kind :precondition :predicate "foothold"
         ...
         (objective
           (hackmode:make-expert-objective
            :id "root-objective" :version "1" :clauses (list precondition goal)
            :limits nil :granted-capabilities '("http-probe")))
         (extension
           (hackmode:make-expert-extension
            :id "recon" :version "2"
            :objective-predicates '("foothold" "final_identity")
            :required-capabilities '("http-probe")
            :authority :passive :strategies '(:symbolic)))
         (registry
           (hackmode:register-expert-extension
            (hackmode:make-expert-extension-registry) extension)))
    (multiple-value-bind (evaluation selection)
        (hackmode:expert-select-extension
         registry objective :authority :active :strategy :symbolic
      ...
      (let ((inspection
              (hackmode:expert-objective-selection-inspection evaluation selection)))
        (assert-equal "root-objective"
                      (hackmode:expert-objective-selection-inspection-objective-id inspection)
                      "selection inspection retains objective identity")
        (assert-equal "1"
                      (hackmode:expert-objective-selection-inspection-objective-version inspection)
        ...
        (let ((objective-id
              ...
              (candidates
                (hackmode:expert-objective-selection-inspection-candidates inspection)))
          (setf (char objective-id 0) #\X
                (char selected-id 0) #\X
                (char (second (first blockers)) 0) #\X
                (char (first (first candidates)) 0) #\X)
          (assert-equal "root-objective"
                        (hackmode:expert-objective-selection-inspection-objective-id inspection)
                        "selection inspection copies objective identity strings")
          (assert-equal "recon"
                        (hackmode:expert-objective-selection-inspection-selected-id inspection)
                        "selection inspection copies selected identity strings")
          (assert-equal '((:precondition "foothold"))
                        (hackmode:expert-objective-selection-inspection-blockers inspection)
                        "selection inspection deeply copies blocker strings")
          (assert-equal '(("recon" "2" :applicable))
                        (hackmode:expert-objective-selection-inspection-candidates inspection)
                        "selection inspection deeply copies candidate strings")))))
  t)
