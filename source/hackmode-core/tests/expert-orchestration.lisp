...
(defun run-expert-orchestration-tests ()
  (let* ((passive (hackmode:make-expert-engine))
    ...
    (let ((dispatch
      ...
      (assert-equal '("dns-resolve" "example.com" "fixture")
                    seen
                    "active dispatch preserves typed provider request")
      (assert-equal action
                    (hackmode:expert-dispatch-request-action dispatch)
                    "dispatch result retains source action")
      (assert-equal :future-token
                    (hackmode:expert-dispatch-request-result dispatch)
