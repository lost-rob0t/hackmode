...
- =source/hackmode-actors/= :: Star-Lang ontology actor system: loads =spec/hackmode-core.star=, spawns the ontology topology onto Sento, and projects operations/assets/evidence into the current StarIntel release.
- =spec/= :: The Hackmode ontology: =hackmode-core.star= (documents, predicates, messages) and one actor declaration per file under =spec/actors/=.
